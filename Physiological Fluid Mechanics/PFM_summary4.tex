...
\section{Turbulence}
\paragraph{The Reynolds number} $\rm Re$ measures the ratio of the inertial force to the viscous force. For \textbf{tube} flow,
\[ 
...
\paragraph{Reynolds averaging} Instantaneous turbulent quantities can be measured, but turbulence is most usefully characterised \textit{statistically} by decomposing flow quantities into mean and fluctuating components: 
\[
    u(t) = \Bar{u} + u'(t).
\]
...
\begin{center}
    ...
    \midrule
    $\Bar{u} = \frac{1}{T} \int_{0}^{T} u \mathrm{d}t$     & $u'(t) = u(t) - \Bar{u}$\\
    \bottomrule
    \end{tabular}
\end{center}
\end{minipage}
...
\]
where the $\tau_{\rm turb} = \rho\overline{u_i' u_j'}$ terms are referred to as the \textbf{Reynolds stresses} (9 terms in total). The total shear stress $\tau_{\rm total} = \tau_{\rm lam} + \tau_{\rm turb}$, where $\tau_{\rm lam} = \mu \frac{\partial \bar{u}}{\partial y}$.
...
\[
    \frac{p_1}{\rho g} + \frac{1}{2g}u_1^2 + h_1 = \frac{p_2}{\rho g} + \frac{1}{2g}u_2^2 + h_2.
\]
...
\[
    \frac{p_1}{\rho g} + \frac{1}{2g}u_1^2 + h_1 = \frac{p_2}{\rho g} + \frac{1}{2g}u_2^2 + h_2 + {\color{red} h_f},
\]
