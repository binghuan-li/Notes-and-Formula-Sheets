...
\begin{table}[H]
    ...
    \midrule
        $p/\rho U^2$    &   Euler number, $\rm Eu$   &   $\frac{\text{pressure force}}{\text{inertia force}}$   &   Problems in which pressure, or pressure differences, are of interest\\
    \midrule
...
The above derivation results in $\nabla^2 \boldsymbol{\omega} = 0$. This leads to the Laplace equation for vorticity in Stokes flow.
\end{tcolorbox}


\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\linewidth]{img/flow_pass_cylinder.eps}
...
\paragraph{Large $\rm Re$ flow ($ \rm Re \gg 1$)} $P_0 = \rho U^2$ and the viscous term eliminated {\color{gray}(hence, the fluid is approximated nearly inviscid)},
\[
